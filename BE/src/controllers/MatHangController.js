...
class MatHangController {
  ...
  async getMatHangById(req, res) {
    try {
      const { id } = req.params;
      const matHang = await this.matHangService.getMatHangById({ maMatHang: id });
  ...
  async createMatHang(req, res) {
    try {
      const matHang = await this.matHangService.createMatHang({ matHangData: req.body });
  ...
  async updateMatHang(req, res) {
    try {
      const { id } = req.params;
      const matHang = await this.matHangService.updateMatHang({ maMatHang: id, matHangData: req.body });
      res.json({ success: true, data: matHang });
    } catch (error) {
      res.status(400).json({ success: false, message: error.message });
    }
  ...
  async deleteMatHang(req, res) {
    try {
      const { id } = req.params;
      await this.matHangService.deleteMatHang({ maMatHang: id });
